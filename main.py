...
def _generate(prompt, duration=None, guidance_scale=7.0):
                    ...
                    if hasattr(obj, 'parameters'):
                        try:
                            ...
                            if p.device.type == 'cpu':
                                print(f"[GEN] Reloading {attr} to GPU...", flush=True)
                                obj.to(_DEVICE)
                                torch.cuda.synchronize()
                                torch.cuda.empty_cache()
                                free_gb = torch.cuda.mem_get_info()[0] / 1e9
                                print(f"[GEN] {attr} reloaded, VRAM free: {free_gb:.2f}GB", flush=True)
    ...
    # Patch VRAM preflight to use lower threshold (RTX 2070 8GB needs this)
    try:
        import acestep.core.generation.handler.generate_music as _gm
        import re as _re
        src = open(_gm.__file__, encoding="utf-8").read()
        # Find and lower the needed_gb threshold
        patched = _re.sub(r'(needed_gb\s*=\s*)0\.[89]\d*', r'\g<1>0.3', src)
        if patched != src:
            open(_gm.__file__, "w", encoding="utf-8").write(patched)
            print("[GEN] Patched VRAM preflight threshold to 0.3GB", flush=True)
    except PermissionError:
        pass
    except Exception as e:
        print(f"[GEN] Preflight patch: {e}", flush=True)
